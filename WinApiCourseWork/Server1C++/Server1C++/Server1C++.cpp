...
string GetCurPos() {
	...
	return "Координаты курсора: x= " + to_string(cursPos.x) + " y= " + to_string(cursPos.y) + "\n";
}
...
void SentMessages(int ind) {
	...
	WriteInLog(GetSystemForLog() + "Отправлено следующее сообщение клиенту " + to_string(ind + 1) + " :\n" + message);
	...
	while (true) {
		...
		if (Errors[ind] != newError) {
			...
			if (send(ArrayConnection[ind], message, sizeof(message), NULL) != SOCKET_ERROR) {

				MessageForConsoleServer();
				WriteInLog(GetSystemForLog() + "Отправлено следующее сообщение клиенту " + to_string(ind + 1) + " :\n" + message);
			}
			else {
				WriteInLog(GetSystemForLog() + "Соединение с клиентом " + to_string(ind + 1) + " прервано\n");
				break;
			}
		}
...
int main(int argc, char* argv[]) {
	...
	HANDLE MyEvent = CreateEvent(NULL, TRUE, FALSE, L"Working1");
	...
	while (true) {
		...
		hPipe = CreateFile(L"\\\\.\\pipe\\MyLogger1",
